...
void rt_kprintf(const char *fmt, ...)
{
	va_list args;
	rt_size_t length;
	static char rt_log_buf[RT_CONSOLEBUF_SIZE];//定义的字符缓冲区

	va_start(args, fmt);
	/* rt_vsnprintf 的返回值 length 表示按照 fmt
		 指定的格式写入到 rt_log_buf 的字符长度 */
	length = rt_vsnprintf(rt_log_buf, sizeof(rt_log_buf) - 1, fmt, args);
  /* 如果 length 超过 RT_CONSOLEBUF_SIZE，则进行截短
     即最多只能输出 RT_CONSOLEBUF_SIZE 个字符 */
